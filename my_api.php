...
class myApi extends Adfox_API
{
    ...
    {
//        if($magName !== 'heroine' || $magName !== 'brodude') return false;

        if ($magName == 'heroine') $name = myApi::DATA['heroineID'];
        if ($magName == 'brodude') $name = myApi::DATA['brodudeID'];
    ...
    function getActiveBanners($campaignName = 'all')
    {
        if ($campaignName = 'all'){
            return $this->getList('activeBanners',
                [
                    'date' => date('Y-m-d'),
                    'limit' => '12',
                ]);
        }
//        if ($campaignName = 'brodude'){
//            return $this->getList('activeBanners',
//                [
//                    'date' => date('Y-m-d'),
//                    'limit' => '12',
